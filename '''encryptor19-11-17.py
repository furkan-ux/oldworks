...
if choice == "E":
	for Tfiles in encFiles:
		if os.path.basename(Tfiles).startswith("(encrpyted)"):
			print "%s is already encrypted" %str(Tfiles)
			pass
		elif Tfiles == os.path.join(os.getcwd(), sys.argv[0]):
			pass
		if not os.path.basename(Tfiles).startswith("'''"):
			encrypt(SHA256.new(password).digest(), str(Tfiles))
			print "Done encrypting %s" %str(Tfiles)
			os.remove(Tfiles)

elif choice == "D":
	filename = "(encrypted)" + raw_input("Enter a fucking file name: ")
	decrypt(SHA256.new(password).digest(), filename)
	print "Done decrypting."
	os.remove(filename)

else:
	print "Lutfen dogru  komut giriniz."
	sys.exit()				
